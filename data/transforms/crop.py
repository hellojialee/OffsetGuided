...
class Crop(Preprocess):
    ...
    def __call__(self, image, anns, meta, mask_miss):
        ...
        image, anns, ltrb = self.crop(image, anns, meta['valid_area'])  # 注意这里ltrb是左上和右下角坐标
        meta['offset'] += ltrb[:2]  # 左上角顶点
    ...
    def area_of_interest(anns, valid_area):
        ...
        此函数是为了能够把所有标注的关键点都包络出来，但是不能超过图像原来的边界，
        即所有关键点50个像素区域范围内都纳入感兴趣区域，然后求出能包围这些关键点区域的范围
        """
        ...
        min_x = min(np.min(ann['keypoints'][ann['keypoints'][:, 2] > 0, 0])
                    for ann in anns_of_interest) - 50  # 关键点50个像素区域范围
        ...
        topleft = (
            max(valid_area[0], min_x),  # 不能超过图像原来的边界
    ...
    def crop(self, image, anns, valid_area):
        ...
        if w > self.long_edge:
            min_x = int(area_of_interest[0])  # 如果裁剪得不合适，会出现训练区域没有关键点，这样造成训练资源浪费
            max_x = int(area_of_interest[0] + area_of_interest[2]) - self.long_edge  # 限定只在感兴趣区域内裁剪
            if max_x > min_x:
        ...
        image = image.crop(ltrb)  # 使用PIL image类中的方法，框(x,y,w,h)
