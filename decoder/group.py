...
class GreedyGroup(object):
    ...
    Args:
        person_thre (float): threshold for pose instance scores.
            If we sort them later, then we can choose a smaller value to detect more person poses.
        ...
        sort_dim (int): sort the person poses by the values at the this axis, which is used in _delete_sort.
            dim 2 means keypoints score, dim 4 means limb score.
        dist_max (float): abandon limbs with delta offsets bigger than dist_max, if joint scales are unavailable.
        use_scale (bool): use the inferred keypoint scales.
    """

    def __init__(self, person_thre, *, del_sort=True, sort_dim=2, dist_max=10, use_scale=False,
    ...
    def group_skeletons(self, limbs, force_complete=False):
        ...
        row_cache = subset_cache((limbs.shape[1], self.n_keypoints, 6))  # (K, 17, 6)
        # row_cache = - 1 * np.ones((limbs.shape[1], self.n_keypoints, 6), dtype=np.float32)  # (K, 17, 6)
...
def subset_cache(shape):
    """
    使用神奇的修饰器@functools.lru_cache，maxsize为最多缓存的次数，这个修饰器用内存保留换取运行速度。
    当第二、三···次调用 相同的输入值shape时，并没有真正执行函数体，
    而是直接返回之前已经出现过的缓存的结果。
    """
    row_cache = - 1 * np.ones(shape, dtype=np.float32)  # (K, 17, 6)
    return row_cache
