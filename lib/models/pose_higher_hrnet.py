...
class PoseHigherResolutionNet(nn.Module):
    ...
    def _make_transition_layer(     # 用来从当前branch中再branch出一个[低分辨率]branch
    ...
    def forward(self, x):
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])
        y_list = self.stage4(x_list)  # the output of stage4 merges heatmaps at 1, 1/2, 1/4, 1/8 scale from 4 branches

        final_outputs = []
        x = y_list[0]
        y = self.final_layers[0](x)  # y's shape： torch.Size([1, 34, 128, 128])
        ...
        for i in range(self.num_deconvs):
            ...
            x = self.deconv_layers[i](x)  # deconv_layers is a ModuleList containing several extra residual blocks
