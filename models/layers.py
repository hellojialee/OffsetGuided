...
class Residual(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.ins != self.outs:
            ...
            # Bn layer is in the middle, so we can do in-plcae += here
        x += residual
...
class Conv(nn.Module):
    # conv block used in our hourglass
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride=1, bn=True,
        ...
        if bn:
            self.conv = nn.Conv2d(inp_dim, out_dim, kernel_size, stride,
                                  ...
                                  dilation=1)
            # Different form TF, momentum default in Pytorch is 0.1, which
            # means the decay rate of old running value
    ...
    def forward(self, x):
        # examine the input channel equals the conv kernel channel
        assert x.size()[
            1] == self.inp_dim, "input channel {} dese not fit kernel " \
                                "channel {}".format(
            x.size()[1],
            self.inp_dim)
        if self.dropout:  # comment these two lines if we do not want to use
            # Dropout layers
...
class DilatedConv(nn.Module):
    ...
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride=1, bn=True,
        ...
        if bn:
            ...
            self.conv = nn.Conv2d(inp_dim, out_dim, kernel_size, stride,
                                  padding=dialation, bias=True,
                                  dilation=dialation)

    def forward(self, x):
        # examine the input channel equals the conv kernel channel
        assert x.size()[1] == self.inp_dim, "input channel {} dese not fit " \
                                            "kernel channel {}".format(
            x.size()[1], self.inp_dim)

        # comment these two lines if we do not want to use Dropout layers
        if self.dropout:
            # p: probability of an element to be zeroed
            x = F.dropout(x, p=0.2, training=self.training,
                          inplace=False)
...
class Hourglass(nn.Module):
    ...
    def __init__(self, depth, nFeat, increase=128, bn=False, resBlock=Residual,
        ...
        # will execute when instantiate the Hourglass object, prepare network's
        # parameters
        self.hg = self._make_hour_glass()
        # no learning parameters, can be used any times repeatedly
        self.downsample = nn.MaxPool2d(2, 2)
        # no learning parameters  # FIXME: 改成反卷积？
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')

    def _make_single_residual(self, depth_id):
        # the innermost conv layer, return as a layer item
    ...
    def _make_lower_residual(self, depth_id):
        ...
        pack_layers = [self.resBlock(self.nFeat + self.increase * depth_id,
                       ...
                       self.convBlock(self.nFeat + self.increase * depth_id,
                                      self.nFeat + self.increase * depth_id,
                                      # ######### Index: 3
                                      bn=self.bn),  # TODO: Conv精细化上采样换成residual?
                       ]
        return pack_layers

    def _make_hour_glass(self):
        """
        Pack conv layers modules of hourglass block
        Returns: conv layers packed in n hourglass blocks
        ...
        for i in range(self.depth):
            ...
            if i == (self.depth - 1):
                # the deepest/longest path need 4 residual blocks
                # res is a list to append elements
                res.append(self._make_single_residual(i))
            # pack conv layers of  every oder of hourglass block
    ...
    def _hour_glass_forward(self, depth_id, x, up_fms):
        ...
        if depth_id == (
                self.depth -
                1):  # except for the highest-order hourglass block
    ...
    def forward(self, x):
        ...
        :return: 5 different scales of feature maps, 128*128, 64*64,
        32*32, 16*16, 8*8
        """
...
class SELayer(nn.Module):
    def __init__(self, inp_dim, reduction=16):
        ...
        """
        assert inp_dim > reduction, \
            f"Make sure your input channel bigger " \
            f"than reduction which equals to {reduction}"
    ...
    # def forward(self, x):  # remove SE layer
    #     return x
